...
angular.module('PrimaryModule').controller('DetailsController', ['$scope', '$timeout', '$http', function ($scope, $timeout, $http) {
	...
	$scope.pastEnforcementClassificationSuccess = function (data) {
		if (data && data.results) {
			...
			var classII = _.filter(data.results, function (r) {
			...
			var classIII = _.filter(data.results, function (r) {
			...
			var classI = _.filter(data.results, function (r) {
	...
	$scope.pastEnforcementStatusSuccess = function (data) {
		if (data && data.results) {
			...
			for (var i = 0; i < data.results.length; i++) {
				if (data.results[i].term == 'ongoing') {
					$scope.enforcementStatusChart.data[0].value = data.results[i].count;
				} else if (data.results[i].term == 'completed') {
					$scope.enforcementStatusChart.data[1].value = data.results[i].count;
				} else if (data.results[i].term == 'terminated') {
					$scope.enforcementStatusChart.data[2].value = data.results[i].count;
				} else {
					$scope.enforcementStatusChart.data[3].value = data.results[i].count;
				}
